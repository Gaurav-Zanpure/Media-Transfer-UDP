...
public class Client {
    ...
    private void receiveAFile() throws IOException, InterruptedException {
        ...
        FileOutputStream fis = new FileOutputStream("E:/File From Server.txt");
    ...
    private void receiveASong() throws IOException, InterruptedException {
        ...
        try {
            fos = new FileOutputStream("E:\\Audio From Server.wav");
            bos = new BufferedOutputStream(fos);
            double nosofpackets = Math.ceil(((int) (new File("E:\\trumpet-cry.wav")).length()) / packetsize);
    ...
    private void receiveAVideo() throws IOException {
        ...
        BufferedOutputStream bos=null;
        try {
            fos = new FileOutputStream("E:\\Video From Server.mp4");
            bos = new BufferedOutputStream(fos);
            double nosofpackets = Math.ceil(((int) (new File("E:\\honey_bee.mp4")).length()) / packetsize);
            byte[] mybytearray = new byte[packetsize];
            DatagramPacket receivePacket = new DatagramPacket(mybytearray, mybytearray.length);
